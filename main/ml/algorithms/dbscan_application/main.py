...
def main():
    try:
        ...
        plt.grid(True)
        plt.savefig(os.path.join(graphs_output_dir, 'dbscan_temp_vs_mq7_simulated.png'))
        plt.close()
        ...
        plt.colorbar(scatter, label='Cluster')
        plt.grid(True)
        plt.savefig(os.path.join(graphs_output_dir, 'dbscan_humidity_vs_mq4_simulated.png'))
        plt.close()
        ...
        pairplot.savefig(os.path.join(graphs_output_dir, 'dbscan_pairplot_simulated.png'))
        plt.close()
        ...
        plt.savefig(os.path.join(graphs_output_dir, 'dbscan_3d_clusters_simulated.png'))
        plt.close()
        ...
        if not df_valid.empty:
            ...
            plt.grid(True)
            plt.tight_layout()
            plt.savefig(os.path.join(graphs_output_dir, 'dbscan_cluster_means_simulated.png'))
            plt.close()
        else:
        ...
        if 'Timestamp' in df.columns:
            ...
            plt.grid(True)
            plt.tight_layout()
            plt.savefig(os.path.join(graphs_output_dir, 'dbscan_cluster_over_time_simulated.png'))
            plt.close()
        else:
