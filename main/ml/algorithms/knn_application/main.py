...
def main():
    try:
        ...
        print(report)
        with open(os.path.join(output_dir, 'knn_classification_report_simulated.txt'), 'w') as f:
            f.write(report)
        ...
        plt.ylabel('Actual')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'knn_confusion_matrix_simulated.png'))
        plt.close()
